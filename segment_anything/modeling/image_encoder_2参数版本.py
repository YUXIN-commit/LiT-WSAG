...
#         x = x.permute(0,3,1,2)
#         B, C, _, _ = x.size()
#         w=self.channel(self.avg_pool(x).view(B, C)).view(B, C, 1, 1)
...
#             B, C, _, _ = y2.size()
#             w1=self.channel(self.avg_pool(y2).view(B, C)).view(B, C, 1, 1)
#             x_channel = w1 * x
...
#         return self.norm(x), self.norm(y1)
...
class Adapter_Layer(nn.Module):
    def __init__(self, embed_dim, mlp_ratio=0.25, norm_layer = nn.LayerNorm, skip_connect=True):
        ...
        for m in self.modules():
            if isinstance(m, (nn.Linear, nn.Conv2d, nn.ConvTranspose2d)):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
        # 假设原通道数是 C 
        self.conv = nn.Conv2d(in_channels=2*embed_dim, out_channels=embed_dim, kernel_size=1) # 1x1卷积层，将通道数还原到 C
        self.relu = nn.ReLU()
    ...
    def forward(self, x, y):
        ...
        if y2 == None:
            ...
            B, C, _, _ = y2.size()
            w1=self.channel(self.avg_pool(y2).view(B, C)).view(B, C, 1, 1)
            z=self.channel(self.avg_pool(x).view(B, C)).view(B, C, 1, 1) * x
            ...
            ##通道拼接
            x_channel=torch.cat((w1*x,z),dim=1)
            # ###卷积将通道数还原到原始通道数 
...
#             nn.BatchNorm2d(channel)
#         )
#         self.avg_pool_se = nn.AdaptiveAvgPool2d(1)
...
#             nn.Sigmoid()
#         )
#         self.fc2_se = nn.Sequential(
#             nn.Conv2d(channel, channel // ratio, 1, bias=False),
#             nn.ReLU(inplace=False),
#             nn.Conv2d(channel // ratio, channel, 1, bias=False),
#             nn.Sigmoid()
#         )
#         self.relu_se = nn.ReLU()
    
#         self.con_AE = nn.Sequential(
...
#         x1 = self.conv1_att(x)
#         #print('g_before',g.shape)
#         g1 = g.permute(0,3,1,2)
#         #print('g_after',g1.shape)
#         g2 = self.conv2_att(g1)
#         add = self.conv3_att(x1+g2)
#         y = self.conv_se(x)
#         # (batch,channel,height,width) (2,512,8,8)
#         b, c, _, _ = y.size()
#         # 全局平均池化 (2,512,8,8) -> (2,512,1,1) -> (2,512)
#         a = self.avg_pool_se(x).view(b, c)
#         # (2,512) -> (2,512//reducation) -> (2,512) -> (2,512,1,1)
#         a = self.fc_se(a).view(b, c, 1, 1)
...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for blk in self.blocks:
            ...
            x ,second_input = blk(x, second_input)
            #x = blk(x)


        x = self.neck(x.permute(0, 3, 1, 2))
        #print('---------------------------------------already load ImageEncoderViT-----------------------------------------')
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor,m) -> torch.Tensor:
        ...
        if self.adapter:
            x_norm = self.norm2(x)
            #y1= self.Adapter(x_norm, x_norm)
            z1, m1 = self.Adapter(x_norm, m)
            #x = x + self.mlp(x_norm) + self.Adapter(x_norm)
            #x = x + self.Adapter(x_norm, self.mlp(x_norm) )
            x = x + self.mlp(x_norm) + z1
        else:
            x = x + self.mlp(self.norm2(x))
